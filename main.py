...
async def reave_time_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    u["grove_loot"] = None
    await save_u(u)
    await update.message.reply_text(f"✅ Роща сброшена для игрока {target_id}! Овечка вернется через 15 секунд.")
...
async def reave_grove_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    u["grove_loot"] = None
    await save_u(u)
    await update.message.reply_text(f"✅ Роща полностью сброшена у игрока {target_id}!")
...
async def generate_grove_loot(update: Update, context: ContextTypes.DEFAULT_TYPE, u: dict):
    ...
    loot = {"coins": coins, "fruit_name": fruit_name, "fruit_field": fruit_field, "is_twin": is_twin}
    u["grove_loot"] = loot
    u["grove_ready"] = True
    u["grove_activity"] = None
    u["grove_activity_finish"] = 0
