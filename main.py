...
async def get_u(uid: int) -> dict:
    ...
    if not user:
        ...
        if any(s in skin for s in ["Шизанутая", "Милая", "Карамельная"]):
            ...
            income_per_hour = 5
        if user.get("buff_watermelon_passive_expires", 0) and now < user.get("buff_watermelon_passive_expires", 0):
            income_per_hour *= 2
        if user.get("buff_kiwi_passive_expires", 0) and now < user.get("buff_kiwi_passive_expires", 0):
            income_per_hour *= 2
...
def main_kb(u: dict = None):
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🎒 Рюкзак", callback_data="backpack"),
...
def get_discount(u: dict) -> int:
    now = time.time()
    discount = 0
    if u.get("buff_blueberry_discount_expires", 0) and now < u.get("buff_blueberry_discount_expires", 0):
        discount += 15
    if u.get("buff_kiwi_discount_expires", 0) and now < u.get("buff_kiwi_discount_expires", 0):
        discount += 25
    if u.get("buff_coconut_discount_expires", 0) and now < u.get("buff_coconut_discount_expires", 0):
        discount += 25
...
def profile_text(u: dict) -> str:
    ...
    if now >= u["harvest"]:
        ...
        timer_line = f"⏳ <b>Следующий сбор через: {format_time(remaining)}</b>"

    return (
        f"<b><i>{display} | 🌿 {satiety}%</i></b>\n"
...
async def backpack_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    if await check_visitor_on_action(update, context):
        return
    u = await get_u(query.from_user.id)
    
    lantern = u.get("lantern", 100)
    equipped = u.get("lantern_equipped", False)
    
    text = "🎒 Рюкзак\n\n• 1 🔦 Фонарик"
    
    if equipped:
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton(f"🔦 {lantern}%", callback_data="backpack_select")],
            [InlineKeyboardButton("⬅️ Назад", callback_data="back")]
        ])
    else:
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("✨ Ничего", callback_data="backpack_select")],
            [InlineKeyboardButton("⬅️ Назад", callback_data="back")]
        ])
...
async def backpack_select(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    u = await get_u(query.from_user.id)
    
    text = "🎒 Выбери, что хочешь надеть?"
    
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("1 🔦 Фонарик", callback_data="backpack_choose_lantern")],
        [InlineKeyboardButton("⬅️ Назад", callback_data="backpack")]
    ])
    
    await query.edit_message_text(text, reply_markup=kb, parse_mode="HTML")
    await query.answer()

async def backpack_choose_lantern(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    u = await get_u(query.from_user.id)
    ...
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton(f"🔦 {lantern}%", callback_data="backpack_wear_lantern")],
        [InlineKeyboardButton("⬅️ Назад", callback_data="backpack")]
    ])
...
async def backpack_wear_lantern(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    u = await get_u(query.from_user.id)
    ...
    if lantern <= 0:
        await query.answer("🔦 Фонарик сломан!", show_alert=True)
        return
    
    u["lantern_equipped"] = True
    await save_u(u)
    
    await query.answer("✅ Фонарик надет!", show_alert=True)
...
# ===== СТРАННЫЙ ТОРГОВЕЦ (ВОЛК) =====

async def show_trader_inline(update: Update, context: ContextTypes.DEFAULT_TYPE, u: dict, edit: bool = True):
    ...
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("💰 Приобрести", callback_data="trader_buy")],
        [InlineKeyboardButton("❌ Отказаться", callback_data="trader_refuse")]
    ])
    if edit and hasattr(update, 'callback_query'):
...
async def show_trader_message(update: Update, context: ContextTypes.DEFAULT_TYPE, u: dict):
    item = u.get("wolf_item")
    if not item:
        return
    text = f"🐺 Странный торговец.\n«Товары на любой вкус»\n{item['name']} – {item['price']} 🐾"
...
async def trader_buy(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    u = await get_u(query.from_user.id)
    item = u.get("wolf_item")
    if not item:
...
async def trader_refuse(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    u = await get_u(query.from_user.id)
    skin = u.get("skin", "🐑 Обычная овечка")
    now = time.time()

    if u.get("buff_wolf_immunity", 0) and now < u.get("buff_wolf_immunity_expires", 0):
...
# ===== МУДРАЯ ЖИТЕЛЬНИЦА (СОВА) =====

async def show_wise_inline(update: Update, context: ContextTypes.DEFAULT_TYPE, u: dict, edit: bool = True):
    ...
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("💰 Отблагодарить", callback_data="wise_pay")],
        [InlineKeyboardButton("❌ Отказаться", callback_data="wise_refuse")]
    ])
    if edit and hasattr(update, 'callback_query'):
        await update.callback_query.edit_message_text(text, reply_markup=keyboard)
    else:
...
async def show_wise_message(update: Update, context: ContextTypes.DEFAULT_TYPE, u: dict):
    advice = u.get("owl_advice")
    if not advice:
        return
    text = f"🦉 Мудрая жительница.\n«С меня совет, с тебя оплата»\n{advice['text']}"
...
async def wise_pay_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    u = await get_u(query.from_user.id)
    if not u.get("owl_active", False):
        await query.answer("❌ Мудрая жительница ушла!", show_alert=True)
        return
    text = (
    ...
    keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("🍏", callback_data="wise_pay_fruit_apple"),
         InlineKeyboardButton("🫐", callback_data="wise_pay_fruit_blueberry"),
         InlineKeyboardButton("🍉", callback_data="wise_pay_fruit_watermelon")],
        [InlineKeyboardButton("🥭", callback_data="wise_pay_fruit_mango"),
         InlineKeyboardButton("🥝", callback_data="wise_pay_fruit_kiwi"),
         InlineKeyboardButton("🥥", callback_data="wise_pay_fruit_coconut")],
        [InlineKeyboardButton("⬅️ Назад", callback_data="wise_back")]
    ])
    await query.edit_message_text(text, reply_markup=keyboard, parse_mode="HTML")

async def wise_pay_fruit(update: Update, context: ContextTypes.DEFAULT_TYPE, fruit_key: str):
    query = update.callback_query
    u = await get_u(query.from_user.id)
    if not u.get("owl_active", False):
        await query.answer("❌ Мудрая жительница ушла!", show_alert=True)
...
async def wise_refuse(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    u = await get_u(query.from_user.id)
    now = time.time()
...
async def wise_back(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    u = await get_u(query.from_user.id)
    if not u.get("owl_active", False):
        await query.answer("❌ Мудрая жительница ушла!", show_alert=True)
        ...
        return
    await show_wise_inline(update, context, u)
...
async def show_bear_inline(update: Update, context: ContextTypes.DEFAULT_TYPE, u: dict, edit: bool = True):
    ...
    if lantern >= 100:
        # Если фонарик уже 100%, мишка не приходит
        u["bear_active"] = False
        await save_u(u)
        return False
    
    text = f"🐻 Ремесленник\n«С тебя копытца, с меня починка»\n🔦 Фонарик ({lantern}%) - {price} 🐾"
    keyboard = InlineKeyboardMarkup([
...
async def bear_repair(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not u.get("bear_active", False):
        await query.answer("❌ Ремесленник ушёл!", show_alert=True)
        return
...
async def check_and_activate_visitor(u: dict, update: Update = None, context: ContextTypes.DEFAULT_TYPE = None) -> bool:
    ...
    if u.get("wolf_active", False) or u.get("owl_active", False) or u.get("bear_active", False):
        return True
    next_visitor = u.get("next_visitor", "trader")
    
    if next_visitor == "trader":
        ...
        if now >= u.get("wolf_auto_time", 0):
            ...
            if update and context:
                await show_trader_message(update, context, u)
            return True
    elif next_visitor == "wise":
        ...
        if now >= u.get("owl_auto_time", 0):
            ...
            await save_u(u)
            if update and context:
                await show_wise_message(update, context, u)
            return True
        ...
        if now >= u.get("bear_auto_time", 0):
            ...
            lantern = u.get("lantern", 100)
            if lantern >= 100:
                # Если фонарик целый, пропускаем мишку
                u["next_visitor"] = "trader"
                u["bear_auto_time"] = 0
                await save_u(u)
                return False
            u["bear_active"] = True
...
async def check_and_show_visitor(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    if u.get("wolf_active", False):
        await show_trader_inline(update, context, u, edit=False)
        return True
    elif u.get("owl_active", False):
        await show_wise_inline(update, context, u, edit=False)
        return True
...
async def check_visitor_on_action(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    await check_and_activate_visitor(u, update, context)
    if u.get("wolf_active", False):
        await show_trader_inline(update, context, u)
        return True
    elif u.get("owl_active", False):
        await show_wise_inline(update, context, u)
        return True
...
async def shear(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    mango_boost = u.get("buff_mango_wool", 0)
    coconut_boost = u.get("buff_coconut_wool", 0)
    ...
    # Повреждаем фонарик при стрижке
    lantern = u.get("lantern", 100)
    if lantern > 0:
        damage = random.randint(2, 5)
        u["lantern"] = max(0, lantern - damage)
...
async def grove_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if u.get("grove_ready", False) and u.get("grove_loot"):
        # Забираем лут
        await collect_grove_loot(update, context, u)
        return
    ...
    if dark:
        ...
        if lantern <= 0:
            await update.message.reply_text("🔦 Фонарик сломан!")
            return
        
        if not equipped:
            await update.message.reply_text("🎒 Фонарик не одет!")
            return
    
    title = "🌳 Тёмная роща" if dark else "🌳 Лесная роща"
    kb = InlineKeyboardMarkup([
...
async def grove_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if dark:
        ...
        if not equipped:
            await query.answer("🎒 Фонарик не одет!", show_alert=True)
            return
...
async def generate_grove_loot(update: Update, context: ContextTypes.DEFAULT_TYPE, u: dict):
    ...
    dark = u.get("grove_is_dark", False)
    
    if dark:
        # Тёмная роща
        coins = random.randint(35, 65)
...
async def collect_grove_loot(update: Update, context: ContextTypes.DEFAULT_TYPE, u: dict):
    loot = u.get("grove_loot")
    
    if not loot or not u.get("grove_ready", False):
        if hasattr(update, 'callback_query'):
            await update.callback_query.answer("❌ Ничего нет!", show_alert=True)
        return
    ...
    if hasattr(update, 'callback_query'):
        await update.callback_query.answer(result_msg, show_alert=True)
        ...
        await grove_cmd(update, context)
...
def main():
    ...
    application.add_handler(CallbackQueryHandler(trader_buy, pattern="^trader_buy$"))
    application.add_handler(CallbackQueryHandler(trader_refuse, pattern="^trader_refuse$"))
    application.add_handler(CallbackQueryHandler(wise_pay_menu, pattern="^wise_pay$"))
    application.add_handler(CallbackQueryHandler(wise_refuse, pattern="^wise_refuse$"))
    application.add_handler(CallbackQueryHandler(wise_back, pattern="^wise_back$"))
    ...
    # Рюкзак
    application.add_handler(CallbackQueryHandler(backpack_cmd, pattern="^backpack$"))
    application.add_handler(CallbackQueryHandler(backpack_select, pattern="^backpack_select$"))
    application.add_handler(CallbackQueryHandler(backpack_choose_lantern, pattern="^backpack_choose_lantern$"))
    application.add_handler(CallbackQueryHandler(backpack_wear_lantern, pattern="^backpack_wear_lantern$"))
    ...
    # Мудрая жительница (фрукты)
    application.add_handler(CallbackQueryHandler(lambda update, context: wise_pay_fruit(update, context, "apple"), pattern="^wise_pay_fruit_apple$"))
    application.add_handler(CallbackQueryHandler(lambda update, context: wise_pay_fruit(update, context, "blueberry"), pattern="^wise_pay_fruit_blueberry$"))
    application.add_handler(CallbackQueryHandler(lambda update, context: wise_pay_fruit(update, context, "watermelon"), pattern="^wise_pay_fruit_watermelon$"))
    application.add_handler(CallbackQueryHandler(lambda update, context: wise_pay_fruit(update, context, "mango"), pattern="^wise_pay_fruit_mango$"))
    application.add_handler(CallbackQueryHandler(lambda update, context: wise_pay_fruit(update, context, "kiwi"), pattern="^wise_pay_fruit_kiwi$"))
    application.add_handler(CallbackQueryHandler(lambda update, context: wise_pay_fruit(update, context, "coconut"), pattern="^wise_pay_fruit_coconut$"))
