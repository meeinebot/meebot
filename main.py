...
async def grove_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if u.get("grove_activity") == "grove":
        ...
        if remaining <= 0:
            ...
            await update.message.reply_text(
                f"🌳 Лесная роща\n\n"
                f"Овечка гуляет и обещает вернуться через: {time_str}",
                parse_mode="HTML"
            )
            return
    ...
    await update.message.reply_text(
        f"🌳 Лесная роща\n\n"
        f"Отличное место для выгула овечки, тут можно найти удивительные вещи!",
        reply_markup=kb,
        parse_mode="HTML"
    )

async def grove_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Кнопка отправки в рощу"""
    query = update.callback_query
    ...
    energy = u.get("energy", 100)
    if energy < 20:
        await query.answer("❌ Недостаточно энергии! Нужно 20 ⚡️", show_alert=True)
        return
    
    if u.get("shearing", 0) == 1 and now < u.get("s_finish", 0):
        await query.answer("✂️ Овечка сейчас стрижётся!", show_alert=True)
        return
...
async def generate_grove_loot(update: Update, context: ContextTypes.DEFAULT_TYPE, u: dict):
    ...
    # 15% - эпические (арбуз, манго)
    # 10% - легендарные (киви, кокос)
    # 75% - редкие (яблоко, черника)
    
    if roll < 15:  # 15% - эпические
        fruit_name, fruit_field = random.choice([
            ("🍉 Арбуз", "inv_watermelon"),
            ("🥭 Манго", "inv_mango")
        ])
    elif roll < 25:  # 10% - легендарные
        fruit_name, fruit_field = random.choice([
            ("🥝 Киви", "inv_kiwi"),
            ("🥥 Кокос", "inv_coconut")
        ])
        ...
        fruit_name, fruit_field = random.choice([
            ("🍏 Яблоко", "inv_apple"),
            ("🫐 Черника", "inv_blueberry")
        ])
    
    # Двойня (35%) - второй такой же фрукт
    is_twin = random.random() < 0.35
...
async def show_grove_result(update: Update, context: ContextTypes.DEFAULT_TYPE, u: dict):
    ...
    if hasattr(update, 'callback_query'):
        await update.callback_query.edit_message_text(
            f"🌳 Лесная роща\n\n"
            f"Овечка вернулась и принесла тебе интересные вещи!",
            reply_markup=kb,
            parse_mode="HTML"
        )
    else:
...
async def grove_collect(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Забирает лут и обновляет сообщение"""
    query = update.callback_query
    u = await get_u(query.from_user.id)
    ...
    # Если двойня - выдаём второй такой же
    if loot.get("is_twin", False) and fruit_name and fruit_field:
        u[fruit_field] = u.get(fruit_field, 0) + 1
        result_msg = f"🍃 Успешно получено! +{coins} 🐾 и {fruit_name} x2"
    else:
        result_msg = f"🍃 Успешно получено! +{coins} 🐾 и {fruit_name}"
...
def main():
    ...
    # ❌ НЕТ ОБРАБОТЧИКА ТЕКСТОВЫХ СООБЩЕНИЙ - БОТ НЕ СПАМИТ!
