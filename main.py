...
WOLF_STEAL_CHANCES = {
    ...
    "🔥 Магмовая овечка": 50,
    "💎 Бриллиантовая овечка": 50,
    "🐚 Жемчужная овечка": 50,
    "👼 Священная овечка": 75,
    "👻 Призрачная овечка": 75,
    "🕯️ Ритуальная овечка": 75
}
...
def format_time(seconds: int) -> str:
    """Форматирует время с правильным склонением (как в роще)"""
    if seconds < 0:
        seconds = 0
...
def get_skin_display(skin: str) -> str:
    """Возвращает название скина с большой буквы"""
    parts = skin.split()
    if len(parts) >= 2:
        return f"{parts[0]} {parts[1].capitalize()} овечка"
    return skin
...
async def shear(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Проверяем, не занята ли овечка в роще
    ...
    if u["shearing"]:
        if now >= u["s_finish"]:
            ...
            # Показываем на 1 минуту меньше (как в роще)
            actual_rem = rem - 60
            if actual_rem < 0:
                actual_rem = 0
        ...
        u["s_finish"] = now + 300
        await save_u(u)
        # Показываем 4 минуты (фактически 4:59)
        await query.answer("✂️ Стрижём твою овечку. ⏳ Процесс займет: 4 минуты", show_alert=True)
...
async def grove_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Лесная роща — новая локация вместо /map"""
    ...
    if u.get("shearing", 0) == 1 and now < u.get("s_finish", 0):
        ...
        await update.message.reply_text(
            f"<i>🌳 Лесная роща</i>\n\n"
            f"<i>✂️ Овечка сейчас стрижётся!\n⏳ Подожди: {time_str}</i>"
        )
        return
    
    # Проверяем, есть ли готовый лут
    if u.get("grove_ready", False) and u.get("grove_loot"):
        await show_grove_result(update, context, u)
        return
    
    # Проверяем, активна ли прогулка
    if u.get("grove_activity") == "grove":
        finish = u.get("grove_activity_finish", 0)
        ...
        if remaining > 0:
            ...
            kb = InlineKeyboardMarkup([
                [InlineKeyboardButton("⏳ Проверить", callback_data="grove_check")]
            ])
            await update.message.reply_text(
                f"<i>🌳 Лесная роща</i>\n\n"
            ...
            # Прогулка закончилась, но лут ещё не сгенерирован
            await generate_grove_loot(update, context, u)
            return
    
    # Показываем главный экран
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("🌲 Отправиться", callback_data="grove_go")]
    ])
    await update.message.reply_text(
        f"<i>🌳 Лесная роща</i>\n\n"
...
async def grove_go(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Отправка овечки в рощу"""
    query = update.callback_query
    u = await get_u(query.from_user.id)
    now = time.time()
    ...
    if u.get("shearing", 0) == 1 and now < u.get("s_finish", 0):
        rem = int(u["s_finish"] - now)
        time_str = format_time(rem)
        await query.answer(f"✂️ Овечка сейчас стрижётся!\n⏳ Подожди: {time_str}", show_alert=True)
        return
    
    # Проверяем, не гуляет ли уже
    if u.get("grove_activity") == "grove":
        finish = u.get("grove_activity_finish", 0)
        if now < finish:
            rem = int(finish - now)
            time_str = format_time(rem)
            await query.answer(f"🌳 Овечка уже гуляет!\n⏳ Вернётся через: {time_str}", show_alert=True)
            return
    
    # Проверяем, есть ли готовый лут
    if u.get("grove_ready", False):
        await show_grove_result(update, context, u)
        return
    
    # Отправляем овечку
    u["grove_activity"] = "grove"
    u["grove_activity_finish"] = now + GROVE_DURATION
    u["grove_ready"] = False
    u["grove_loot"] = None
    await save_u(u)
    
    time_str = format_time(GROVE_DURATION)
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("⏳ Проверить", callback_data="grove_check")]
    ])
    await query.edit_message_text(
    ...
    await query.answer("🌳 Овечка отправилась в рощу!", show_alert=True)

async def grove_check(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Проверка статуса прогулки"""
    query = update.callback_query
    u = await get_u(query.from_user.id)
    now = time.time()
    ...
    if u.get("grove_activity") == "grove":
        ...
        if remaining > 0:
            ...
            await query.answer("🌳 Овечка ещё гуляет...")
            return
        else:
            await generate_grove_loot(update, context, u)
            return
    
    # Если ничего не активно — показываем главный экран
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("🌲 Отправиться", callback_data="grove_go")]
    ])
    await query.edit_message_text(
...
async def generate_grove_loot(update: Update, context: ContextTypes.DEFAULT_TYPE, u: dict):
    ...
    for chance, f_name, f_field in GROVE_LOOT_TABLE:
        cumulative += chance
        if roll < cumulative:
    ...
    if fruit_name:
        u["grove_loot"] = {"type": "fruit", "name": fruit_name, "field": inv_field}
    else:
        reward = random.randint(5, 35)
        u["grove_loot"] = {"type": "coins", "amount": reward}
...
async def show_grove_result(update: Update, context: ContextTypes.DEFAULT_TYPE, u: dict):
    """Показывает результат прогулки"""
    query = update.callback_query if hasattr(update, 'callback_query') else None
    loot = u.get("grove_loot")
    
    if not loot:
        # Если лута нет — сбрасываем и показываем главный экран
        u["grove_ready"] = False
        await save_u(u)
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("🌲 Отправиться", callback_data="grove_go")]
    ...
    if loot["type"] == "fruit":
        result_msg = f"🐑 Овечка вернулась и принесла тебе кое что интересное!"
        reward_msg = f"🍃 Вот что принесла тебе твоя овечка!\n+ {loot['name']}"
    else:
        result_msg = f"🐑 Овечка вернулась и принесла тебе кое что интересное!"
        reward_msg = f"🍃 Вот что принесла тебе твоя овечка!\n+ {loot['amount']} 🐾 копытц"
    
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("🍃 Посмотреть", callback_data="grove_collect")]
    ])
    ...
    if query:
        ...
        await query.answer("🌳 Овечка вернулась!", show_alert=True)

async def grove_collect(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Сбор лута после прогулки"""
    query = update.callback_query
    u = await get_u(query.from_user.id)
    loot = u.get("grove_loot")
    ...
    if loot["type"] == "fruit":
        ...
        old_level = add_xp(u, random.randint(2, 5))
        await query.answer(f"🌳 Вот что принесла тебе твоя овечка!\n+ {loot['name']}", show_alert=True)
    else:
        u["balance"] += loot["amount"]
        old_level = add_xp(u, random.randint(2, 5))
        await query.answer(f"🌳 Вот что принесла тебе твоя овечка!\n+ {loot['amount']} 🐾 копытц", show_alert=True)
...
def main():
    ...
    application.add_handler(CommandHandler("grove", grove_cmd))  # Новая команда!
    ...
    # Роща
    application.add_handler(CallbackQueryHandler(grove_go, pattern="^grove_go$"))
    application.add_handler(CallbackQueryHandler(grove_check, pattern="^grove_check$"))
