...
# Функция для получения времени сброса (12:00:00 по МСК)
def get_reset_time():
    now = datetime.now()
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "dig":
        await query.edit_message_text("⛏️ Ты копаешь руду... (функция в разработке)")
    elif query.data == "workbench":
        await query.edit_message_text("🛠️ Верстак откроется позже.")
    elif query.data == "prestige":
        await query.edit_message_text("🏆 Престиж пока не сделан.")
    else:
