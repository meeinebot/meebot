...
async def get_u(uid: int) -> dict:
    ...
    if not user:
        ...
        energy_regen = int(time_passed // 3600)
        if energy_regen > 0:
...
async def daily_claim(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if reward["type"] == "coins":
        ...
        await save_u(u)
        await query.answer(f"💰 Успешно получено! +{amount} 🐾", show_alert=True)
...
async def daily_fruit_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Только смайлик в шов-алерте
    fruit_emoji = fruit_name.split()[0]
    await query.answer(f"💰 Успешно получено! +{fruit_emoji}", show_alert=True)
