...
def get_egg_reward(current_skin: str):
    ...
    weights = []
    if skin_level == 0:  # Обычная
        weights = [40, 30, 20, 10]
    elif skin_level == 1:  # Редкая
        weights = [40, 30, 20, 10]
    elif skin_level == 2:  # Эпическая
        weights = [0, 40, 30, 10]
    elif skin_level == 3:  # Легендарная
        weights = [0, 0, 40, 10]
    elif skin_level == 4:  # Мифическая
        weights = [0, 0, 0, 40]
    else:
        weights = [40, 30, 20, 10]
    ...
    if skin_level > rarity_index:
        ...
        if fruits:
            ...
            return {"type": "fruit", "name": None, "skin": None}
    else:
