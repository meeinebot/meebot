...
async def get_u(uid: int) -> dict:
    ...
    if not user:
        ...
        # ===== ВОССТАНОВЛЕНИЕ ЭНЕРГИИ (1% В ЧАС) =====
        energy_update = user.get("energy_update", now)
        time_passed = now - energy_update
        energy_regen = int(time_passed // 3600)  # 1 энергия в час
        if energy_regen > 0:
...
async def shear(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if u["shearing"]:
        if now >= u["s_finish"]:
            ...
            # ===== ТРАТИМ ЭНЕРГИЮ (5-10) =====
            u["energy"] = max(0, u["energy"] - random.randint(5, 10))
...
async def grove_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    energy = u.get("energy", 100)
    if energy < 15:
        await update.message.reply_text("❌ Недостаточно энергии! Нужно 15 ⚡️")
        return
...
async def grove_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    energy = u.get("energy", 100)
    if energy < 15:
        await query.answer("❌ Недостаточно энергии! Нужно 15 ⚡️", show_alert=True)
        return
...
async def grove_collect(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ===== ТРАТИМ ЭНЕРГИЮ (15-25) =====
    u["energy"] = max(0, u["energy"] - random.randint(15, 25))
