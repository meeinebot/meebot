...
async def sheep_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Проверяем принудительный вызов Волка
    if u.get("wolf_forced", 0) == 1:
        forced_item_name = u.get("wolf_forced_item", "")
        if forced_item_name:
            item = {"name": forced_item_name, "price": None}
            for wolf_item in WOLF_ITEMS:
                if wolf_item["name"] == forced_item_name:
                    item["price"] = wolf_item["price"]
                    break
...
async def wolf_refuse(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if random.random() * 100 < steal_chance:
        ...
        await query.answer("🐺 Странный торговец ушёл...", show_alert=True)
...
async def wolf_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    u["wolf_forced"] = 1
    u["wolf_forced_item"] = fruit_map[fruit_emoji]["name"]
    await save_u(u)
