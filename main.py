...
async def wolf_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not u:
        ...
        return
    # Принудительно вызываем Волка (устанавливаем время в прошлое)
    u["wolf_last_offer"] = time.time() - 86400
    await save_u(u)
    # Сохраняем товар в контексте
    context.user_data["wolf_item"] = {"name": fruit_map[fruit_emoji], "price": 0}
    context.user_data["wolf_active"] = True
    # Никаких уведомлений игроку не отправляем
