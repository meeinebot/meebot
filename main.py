...
def xp_bar(xp: int, needed: int, length: int = 10) -> str:
    filled = min(length, int(length * xp / needed)) if needed else 0
...
def profile_text(u: dict) -> str:
    ...
    if name == "Овечка":
        name = skin_parts[1] if len(skin_parts) > 1 else "Обычная"
    level = u.get("level", 1)
    balance = u["balance"]
    xp = u.get("xp", 0)
...
async def market_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    kb = InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🎰 Лотерейный киоск", callback_data="lottery")]
    ])
...
async def lottery_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    price = 49 * (100 - discount) // 100
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("🎰 Испытать удачу", callback_data="lottery_spin")],
        [InlineKeyboardButton("⬅️ Назад", callback_data="market_main")]
    ])
    await query.edit_message_text(
        f"<b>🎰 Лотерейный киоск</b>\n<i>Курс: 1 билет = {price} 🐾</i>",
        reply_markup=kb,
        parse_mode="HTML"
    )

async def lottery_spin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    if await check_visitor_on_action(update, context):
        return
    u = await get_u(query.from_user.id)
    ...
    if u["balance"] < price:
        ...
        return
    u["balance"] -= price
    win = random.randint(15, 75)
    u["balance"] += win
    old_level = add_xp(u, random.randint(2, 5))
    await save_u(u)
    await check_level_up(update, context, u, old_level)
    await query.answer(f"🎰 Ты испытал удачу! Твой выигрыш составил: {win} 🐾", show_alert=True)
...
async def map_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    buttons.append([InlineKeyboardButton("❌ Закончить прогулку", callback_data="map_back")])
...
async def map_back(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    u = await get_u(query.from_user.id)
    name = u.get("name", "Овечка")
    level = u.get("level", 1)
...
def main():
    ...
    application.add_handler(CallbackQueryHandler(lottery_menu, pattern="^lottery$"))
    application.add_handler(CallbackQueryHandler(lottery_spin, pattern="^lottery_spin$"))
    ...
    application.add_handler(CallbackQueryHandler(map_back, pattern="^map_back$"))
