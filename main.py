...
MONGO_URI = "mongodb+srv://sheep_bot:meewoolbotsheep199@cluster0.jjp6pia.mongodb.net/?appName=Cluster0"
OWNER_ID = 1864104580
...
async def sheep_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Проверка Волка (раз в 24 часа, шанс 25%)
    if u.get("wolf_last_offer", 0) < now - 86400 and random.random() < 0.25:
...
async def wolf_buy(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif "Легендарное яйцо" in item['name']:
        u['inv_legendary_egg'] = u.get('inv_legendary_egg', 0) + 1
    await save_u(u)
...
async def wolf_refuse(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if random.random() * 100 < steal_chance:
        ...
        await save_u(u)
        await query.answer(f"🐺 Странный торговец схватил твою {skin} и сбежал!", show_alert=True)
    else:
