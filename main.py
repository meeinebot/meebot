...
async def show_wolf_message(update: Update, context: ContextTypes.DEFAULT_TYPE, u: dict):
    """Показывает волка новым сообщением"""
    item = u.get("wolf_item")
    if not item:
        return
...
async def check_and_activate_wolf(u: dict, update: Update = None, context: ContextTypes.DEFAULT_TYPE = None) -> bool:
    """Проверяет и активирует волка автоматически"""
    now = time.time()
    ...
    if u.get("wolf_auto_time", 0) == 0:
        auto_time = now + random.randint(12 * 3600, 36 * 3600)
        u["wolf_auto_time"] = auto_time
        await save_u(u)
        return False
    
    if now >= u.get("wolf_auto_time", 0) and not u.get("wolf_active", False):
...
async def check_wolf_on_action(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    """Проверяет волка при любом действии"""
    query = update.callback_query
    u = await get_u(query.from_user.id)
    
    await check_and_activate_wolf(u, update, context)
...
async def treats_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    if await check_wolf_on_action(update, context):
        return
