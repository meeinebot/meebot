...
async def grove_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if u.get("grove_activity") == "grove":
        ...
        if remaining <= 0:
            ...
            if roll < 70:
                u["balance"] += coins
                msg = f"🌳 Овечка вернулась! +{coins} 🐾"
            elif roll < 85:
                fruit, field = random.choice([("🍏 Яблоко", "inv_apple"), ("🫐 Черника", "inv_blueberry")])
                u[field] = u.get(field, 0) + 1
                msg = f"🌳 Овечка вернулась! +{fruit}"
            elif roll < 95:
                fruit, field = random.choice([("🍉 Арбуз", "inv_watermelon"), ("🥭 Манго", "inv_mango")])
                u[field] = u.get(field, 0) + 1
                msg = f"🌳 Овечка вернулась! +{fruit}"
            else:
                fruit, field = random.choice([("🥝 Киви", "inv_kiwi"), ("🥥 Кокос", "inv_coconut")])
                u[field] = u.get(field, 0) + 1
                msg = f"🌳 Овечка вернулась! +{fruit}"
