...
# --- НОВЫЕ ФУНКЦИИ ДЛЯ ТАЛАНТОВ ---
def get_shear_cooldown(u: dict) -> int:
    """Рассчитывает время стрижки с учетом уровня Острых ножниц"""
    base_time = 12 * 3600
    level = u.get("talents", {}).get("golden_shears", 0)
    reduction = level * 36 * 60
    return max(6 * 3600, base_time - reduction)

def get_wool_price(u: dict) -> int:
    """Рассчитывает цену шерсти с учетом уровня Шерстяного люкса"""
    base_price = 10
    level = u.get("talents", {}).get("luxury_wool", 0)
    return base_price + level

def xp_to_next(level: int) -> int:
    """Новая формула опыта: 50, 75, 100, 125..."""
    if level >= 30:
        return float('inf')
    return 25 * level + 25
...
async def get_u(uid: int) -> dict:
    ...
    if not user:
        ...
        if "talents" in user and "appetizing_feed" in user["talents"]:
            del user["talents"]["appetizing_feed"]
            available = [t for t in ["golden_shears", "luxury_wool"] 
                        if user["talents"].get(t, 0) < 10]
            if available:
                chosen = random.choice(available)
                user["talents"][chosen] = user["talents"].get(chosen, 0) + 1
        
        if "inv_compass" not in user:
            user["inv_compass"] = 1
        if "compass_equipped" not in user:
            user["compass_equipped"] = False
...
def main_kb(u: dict = None):
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("✂️ Стрижка", callback_data="shear"),
         InlineKeyboardButton("⭐️ Прогресс", callback_data="progress")],
...
async def sheep_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if u.get("pending_level_up", False):
        await show_level_up(update, context, u)
        return
...
async def progress_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    next_level = level + 1
    golden = u.get("talents", {}).get("golden_shears", 0)
    luxury = u.get("talents", {}).get("luxury_wool", 0)
    
    text = f"""⭐️ Твой прогресс
`···············`
• ✂️ Острые ножницы: {golden}
• 🧶 Шерстяной люкс: {luxury}
`···············`
✨ {xp}/{next_xp} ({level} → {next_level})"""
...
# --- ТАЛАНТЫ ---
async def show_level_up(update: Update, context: ContextTypes.DEFAULT_TYPE, u: dict):
    level = u.get("level", 0)
    talents = ["golden_shears", "luxury_wool"]
    ...
    if not available:
        ...
        if isinstance(update, Update) and update.callback_query:
            await update.callback_query.answer("🎉 Все таланты изучены до максимума! +1 🍏 Яблоко", show_alert=True)
            await update.callback_query.edit_message_text(profile_text(u), reply_markup=main_kb(u))
        else:
            await update.message.reply_text(
                f"🎉 Овечка достигла {level} уровня! +1 🍏 Яблоко\n⭐️ Все таланты изучены до максимума!",
    ...
    text = f"""🎉 Овечка достигла {level} уровня! +1 🍏 Яблоко
⭐️ Нажми, чтобы изучить новый талант!"""
...
async def learn_talent(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    talents = ["golden_shears", "luxury_wool"]
    talent_names = {
        "golden_shears": "✂️ Острые ножницы",
        "luxury_wool": "🧶 Шерстяной люкс"
    }
    ...
    if not available:
        ...
        u["pending_level_up"] = False
        await save_u(u)
        await query.answer("🎉 Все таланты изучены до максимума! +1 🍏 Яблоко", show_alert=True)
    ...
    await query.answer(
        f"✨ Изучен новый талант! {talent_names[chosen]}. ({old_level} → {new_level})",
        show_alert=True
    )
    await query.edit_message_text(profile_text(u), reply_markup=main_kb(u))
...
async def inventory_main(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if u.get("inv_flashlight", 0) > 0 and not u.get("flashlight_broken", False):
        ...
        total += u["inv_flashlight"]
    
    if u.get("inv_compass", 0) > 0:
        items.append(f"• {u['inv_compass']} 🧭 Компас")
        total += u["inv_compass"]
    ...
    if u.get("flashlight_equipped", False) and not u.get("flashlight_broken", False):
        ...
        bottom_button = f"🔦 {durability}%"
    elif u.get("compass_equipped", False):
        bottom_button = "🧭 100%"
    else:
        bottom_button = "✨ Ничего"
...
• Общее: {total}"""
    ...
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton(bottom_button, callback_data="inventory_select")],
        [InlineKeyboardButton("⬅️ Назад", callback_data="back")]
    ])
...
async def inventory_select(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    u = await get_u(query.from_user.id)
    if await check_visitor_on_action(update, context):
        return
    
    has_flashlight = u.get("inv_flashlight", 0) > 0 and not u.get("flashlight_broken", False)
    has_compass = u.get("inv_compass", 0) > 0
    
    if not has_flashlight and not has_compass:
        text = "🎒 Выбери, что хочешь надеть:"
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("⬅️ Назад", callback_data="inventory_main")]
        ])
    ...
    if u.get("flashlight_equipped", False) and not u.get("flashlight_broken", False):
        durability = u.get("flashlight_durability", 100)
        equipped_text = f"• ✨ Надето: 🔦 {durability}%"
    elif u.get("compass_equipped", False):
        equipped_text = "• ✨ Надето: 🧭 100%"
    
    row = []
    if has_flashlight:
        row.append(InlineKeyboardButton(f"🔦 {u['inv_flashlight']} Фонарик", callback_data="wear_flashlight"))
    if has_compass:
        row.append(InlineKeyboardButton(f"🧭 {u['inv_compass']} Компас", callback_data="wear_compass"))
    
    bottom_row = []
    if equipped_text:
        bottom_row.append(InlineKeyboardButton("❌ Снять", callback_data="unequip_item"))
    bottom_row.append(InlineKeyboardButton("⬅️ Назад", callback_data="inventory_main"))
    
    kb = InlineKeyboardMarkup([row, bottom_row] if bottom_row else [row])
    
    text = f"🎒 Выбери, что хочешь надеть:\n{equipped_text}" if equipped_text else "🎒 Выбери, что хочешь надеть:"
    
    await query.edit_message_text(text, reply_markup=kb)

async def wear_flashlight(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    u = await get_u(query.from_user.id)
    
    if u.get("inv_flashlight", 0) <= 0 or u.get("flashlight_broken", False):
        await query.answer("❌ У тебя нет этого предмета!", show_alert=True)
        return
    
    u["flashlight_equipped"] = True
    u["compass_equipped"] = False
    await save_u(u)
...
async def wear_compass(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    u = await get_u(query.from_user.id)
...
async def shear(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    u["wool"] += gain
    cooldown = get_shear_cooldown(u)
    u["harvest"] = now + cooldown
...
async def inventory(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    apple = u.get("inv_apple", 0)
    blueberry = u.get("inv_blueberry", 0)
    watermelon = u.get("inv_watermelon", 0)
    mango = u.get("inv_mango", 0)
    kiwi = u.get("inv_kiwi", 0)
    coconut = u.get("inv_coconut", 0)
    
    text = f"📦 Склад\n🍏 {apple} |  {blueberry} | 🍉 {watermelon}\n🥭 {mango} | 🥝 {kiwi} | 🥥 {coconut}\n"
    
    buttons = []
    row = []
    if apple > 0:
        row.append(InlineKeyboardButton("🍏", callback_data="use_fruit_apple"))
    if blueberry > 0:
        row.append(InlineKeyboardButton("", callback_data="use_fruit_blueberry"))
    if watermelon > 0:
        row.append(InlineKeyboardButton("🍉", callback_data="use_fruit_watermelon"))
    if len(row) > 0:
        buttons.append(row)
        row = []
    if mango > 0:
        row.append(InlineKeyboardButton("🥭", callback_data="use_fruit_mango"))
    if kiwi > 0:
        row.append(InlineKeyboardButton("🥝", callback_data="use_fruit_kiwi"))
    if coconut > 0:
        row.append(InlineKeyboardButton("🥥", callback_data="use_fruit_coconut"))
    if len(row) > 0:
        buttons.append(row)
...
async def use_fruit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    u = await get_u(query.from_user.id)
    
    fruit_key = query.data.split("_")[-1]
    inv_field = f"inv_{fruit_key}"
    
    if u.get(inv_field, 0) <= 0:
        await query.answer("❌ У тебя нет этого фрукта!", show_alert=True)
        return
    ...
    u["satiety"] = min(100, u.get("satiety", 100) + restore)
    ...
    await save_u(u)
    await query.answer(f"🍎 Ты съел фрукт! Сытость +{restore}%", show_alert=True)
...
async def sell_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    price_per_wool = get_wool_price(u)
    v = u["wool"] * price_per_wool
    u["balance"] += v
    u["wool"] = 0
...
async def wolf_scare(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if u.get("inv_flashlight", 0) <= 0 or u.get("flashlight_broken", False):
        await query.answer("❌ У тебя нет фонарика!", show_alert=True)
        return
    ...
    if durability == 0:
        ...
        await query.answer("🔦 Фонарик сломался!", show_alert=True)
    else:
        await save_u(u)
...
def main():
    ...
    application.add_handler(CallbackQueryHandler(progress_cmd, pattern="^progress$"))
    ...
    application.add_handler(CallbackQueryHandler(inventory_select, pattern="^inventory_select$"))
    application.add_handler(CallbackQueryHandler(wear_flashlight, pattern="^wear_flashlight$"))
    application.add_handler(CallbackQueryHandler(wear_compass, pattern="^wear_compass$"))
    application.add_handler(CallbackQueryHandler(unequip_item, pattern="^unequip_item$"))
    ...
    application.add_handler(CallbackQueryHandler(use_fruit, pattern="^use_fruit_"))
