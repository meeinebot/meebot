...
async def get_u(uid: int) -> dict:
    ...
    if not user:
        ...
        if any(s in skin for s in ["Шизанутая", "Милая", "Карамельная"]):
            ...
            income_per_hour = 5
        if user.get("buff_watermelon_passive", 0) and now < user.get("buff_watermelon_passive_expires", 0):
            income_per_hour *= 2
        if user.get("buff_kiwi_passive", 0) and now < user.get("buff_kiwi_passive_expires", 0):
            income_per_hour *= 2
...
def get_current_effect(u: dict) -> str:
    ...
    if u.get("buff_apple_immunity", 0) and now < u.get("buff_apple_immunity_expires", 0):
        effects.append("🍏 Яблоко (иммунитет)")
    if u.get("buff_apple_wool", 0):
        effects.append("🍏 Яблоко (+5 шерсти)")
    # Черника
    if u.get("buff_blueberry_immunity", 0) and now < u.get("buff_blueberry_immunity_expires", 0):
        effects.append("🫐 Черника (иммунитет)")
    if u.get("buff_blueberry_discount", 0) and now < u.get("buff_blueberry_discount_expires", 0):
        effects.append("🫐 Черника (скидка 10%)")
    # Арбуз
    if u.get("buff_watermelon_immunity", 0) and now < u.get("buff_watermelon_immunity_expires", 0):
        effects.append("🍉 Арбуз (иммунитет)")
    if u.get("buff_watermelon_passive", 0) and now < u.get("buff_watermelon_passive_expires", 0):
        effects.append("🍉 Арбуз (пассивка ×2)")
    # Манго
    if u.get("buff_mango_immunity", 0) and now < u.get("buff_mango_immunity_expires", 0):
        effects.append("🥭 Манго (иммунитет)")
    if u.get("buff_mango_wool", 0):
        effects.append("🥭 Манго (15-25 шерсти)")
    # Киви
    if u.get("buff_kiwi_immunity", 0) and now < u.get("buff_kiwi_immunity_expires", 0):
        effects.append("🥝 Киви (иммунитет)")
    if u.get("buff_kiwi_passive", 0) and now < u.get("buff_kiwi_passive_expires", 0):
        effects.append("🥝 Киви (пассивка ×2)")
    if u.get("buff_kiwi_discount", 0) and now < u.get("buff_kiwi_discount_expires", 0):
        effects.append("🥝 Киви (скидка 10%)")
    # Кокос
    if u.get("buff_coconut_immunity", 0) and now < u.get("buff_coconut_immunity_expires", 0):
        effects.append("🥥 Кокос (иммунитет)")
    if u.get("buff_coconut_wool", 0):
        effects.append("🥥 Кокос (15-25 шерсти)")
    if u.get("buff_coconut_discount", 0) and now < u.get("buff_coconut_discount_expires", 0):
        effects.append("🥥 Кокос (скидка 10%)")
    if effects:
        return ", ".join(effects)
    return "🚫 Неактивен"
...
async def wolf_buy(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Разбираем товар (одиночный или набор)
    if "+" in item['name']:
...
async def use_fruit(update: Update, context: ContextTypes.DEFAULT_TYPE, fruit_key: str, inv_field: str):
    ...
    if fruit_key == "apple":
        ...
        u["buff_kiwi_passive"] = 1
        u["buff_kiwi_passive_expires"] = now + 12 * 3600
        u["buff_kiwi_discount"] = 1
...
async def shear(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if u['shearing']:
        if now >= u['s_finish']:
            ...
            u['wool'] += gain
            u['shearing'] = 0
            base_cooldown = 12 * 3600
            u['harvest'] = now + base_cooldown
            # Сбрасываем одноразовые баффы на шерсть
            u["buff_apple_wool"] = 0
            u["buff_mango_wool"] = 0
...
async def give_fruit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    if user_id != OWNER_ID:
        return
    args = context.args
    if len(args) < 3:
        await update.message.reply_text("❌ Использование: /give <фрукт> <количество> <user_id>\nПример: /give 🍏 5 123456789")
        return
    try:
        fruit_emoji = args[0]
        amount = int(args[1])
    ...
    field = fruit_map[fruit_emoji]
    u[field] = u.get(field, 0) + amount
    await save_u(u)
    await update.message.reply_text(f"✅ Добавлено {amount} {fruit_emoji} игроку с ID {target_id}.")
...
def main():
    ...
    application.add_handler(CommandHandler("give", give_fruit))
