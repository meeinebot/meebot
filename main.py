...
async def reave_grove_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Сброс рощи у игрока (только для админа)"""
    if update.effective_user.id != ADMIN_ID:
        return
    
    args = context.args
    if len(args) != 1:
        await update.message.reply_text("❌ Использование: /reave_grove <user_id>\nПример: /reave_grove 401025920")
        return
    ...
    await save_u(u)
    await update.message.reply_text(f"✅ Роща сброшена у игрока {target_id}!")
...
async def generate_grove_loot(update: Update, context: ContextTypes.DEFAULT_TYPE, u: dict):
    # Если лут уже есть — не генерируем заново
    if u.get("grove_ready", False) and u.get("grove_loot"):
        await show_grove_result(update, context, u)
        return
