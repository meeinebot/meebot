...
WOLF_ENERGY_LOSS = {
    "🐑 Обычная овечка": 50,
    "🏡 Деревенская овечка": 35,
    "🏖️ Пляжная овечка": 35,
    "💤 Сонная овечка": 35,
    "💥 Шизанутая овечка": 25,
    "🎀 Милая овечка": 25,
    "🍭 Карамельная овечка": 25,
    "🔥 Магмовая овечка": 15,
    "💎 Бриллиантовая овечка": 15,
    "🐚 Жемчужная овечка": 15,
    "👼 Священная овечка": 5,
    "👻 Призрачная овечка": 5,
    "🕯️ Ритуальная овечка": 5
}
...
GROVE_DURATION = 90 * 60  # 90 минут
ADMIN_ID = 1864104580  # ВАШ ID
...
async def get_u(uid: int) -> dict:
    ...
    if not user:
        ...
        energy_update = user.get("energy_update", now)
        time_passed = now - energy_update
        energy_regen = int(time_passed // 300)
        if energy_regen > 0:
            user["energy"] = min(100, user.get("energy", 100) + energy_regen)
            user["energy_update"] = now
...
def profile_text(u: dict) -> str:
    ...
    if now >= u["harvest"]:
        ...
        timer_line = f"⏳ <i>До следующего сбора: {format_time(remaining)}</i>"
    
    return (
        f"<i>{skin_display} | ⚡️{energy}%</i>\n"
...
async def reave_time_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(args) != 1:
        await update.message.reply_text(
            "❌ Использование: /reave_time <user_id>\n"
            "Пример: /reave_time 5020105592"
        )
        return
    ...
    await update.message.reply_text(
        f"✅ Роща сброшена для игрока {target_id}!\n"
        f"⏳ Овечка вернется через 15 секунд.\n"
        f"Напиши /grove чтобы проверить статус."
    )

# ============= ЕЖЕДНЕВНЫЙ БОНУС =============

DAILY_REWARDS = [
    {"day": 1, "type": "choice", "items": ["🍏 Яблоко", "🫐 Черника"], "fields": ["inv_apple", "inv_blueberry"]},
    ...
    {"day": 7, "type": "choice", "items": ["🥝 Киви", "🥥 Кокос"], "fields": ["inv_kiwi", "inv_coconut"]}
]
...
async def daily_fruit_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer(f"💰 Награда успешно получена! +{fruit_name}", show_alert=True)
    await query.edit_message_text(
        f"<i>🎯 Ежедневный бонус</i>\n<i>⭐️ Серия наград: {u.get('daily_streak', 0)}</i>",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("💰 Получить награду", callback_data="daily_claim")]
...
async def owl_pay_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not u.get("owl_active", False):
        ...
        return
    text = (
        f"<i>⚡️ Выбери, чем хочешь отблагодарить.</i>\n"
...
async def wolf_refuse(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    energy_loss = WOLF_ENERGY_LOSS.get(skin, 50)
    u["energy"] = max(0, u["energy"] - energy_loss)
...
async def use_fruit(update: Update, context: ContextTypes.DEFAULT_TYPE, fruit_key: str, inv_field: str):
    ...
    u["energy"] = min(100, energy + energy_restore)
...
async def sell_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    u = await get_u(query.from_user.id)
    if u["wool"] <= 0:
        await query.answer("❌ Недостаточно шерсти!", show_alert=True)
        return
    v = u["wool"] * 10
    u["balance"] += v
    u["wool"] = 0
...
async def shear(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    energy = u.get("energy", 100)
    if energy < 5:
        await query.answer("❌ Недостаточно энергии!", show_alert=True)
        return
    
    if u.get("grove_activity") == "grove":
        finish = u.get("grove_activity_finish", 0)
        if now < finish:
            await query.answer("🌲 Овечка занята. ⏳ Подожди ее прихода!", show_alert=True)
            return
    ...
    if u["shearing"]:
        if now >= u["s_finish"]:
            ...
            await save_u(u)
            await query.answer(f"🐑 Овечка успешно пострижена!\nПолучено: 🧶 {gain}", show_alert=True)
            await query.edit_message_text(profile_text(u), reply_markup=main_kb(u), parse_mode="HTML")
        else:
            rem = int(u["s_finish"] - now)
            time_str = format_time(rem)
            await query.answer(f"✂️ Стрижём твою овечку. ⏳ Процесс займет: {time_str}", show_alert=True)
    elif now < u["harvest"]:
        rem = int(u["harvest"] - now)
        time_str = format_time(rem)
        await query.answer(f"❌ Шерсть еще не готова к сбору!\n⏳ Подожди: {time_str}", show_alert=True)
    else:
        u["shearing"] = 1
        u["s_finish"] = now + 300
        await save_u(u)
        await query.answer("✂️ Стрижём твою овечку. ⏳ Процесс займет: 5 минут", show_alert=True)
...
async def grove_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    energy = u.get("energy", 100)
    if energy < 20:
        await update.message.reply_text("❌ Недостаточно энергии! Нужно 20 ⚡️")
        return
    
    if u.get("shearing", 0) == 1 and now < u.get("s_finish", 0):
        rem = int(u["s_finish"] - now)
        time_str = format_time(rem)
        await update.message.reply_text(
            f"✂️ Овечка сейчас стрижётся!\n⏳ Подожди: {time_str}",
            parse_mode="HTML"
        )
        return
    
    # ===== ПРОВЕРЯЕМ ГОТОВЫЙ ЛУТ =====
    if u.get("grove_ready", False) and u.get("grove_loot"):
        await show_grove_result(update, context, u)
        return
    
    # ===== ПРОВЕРЯЕМ АКТИВНУЮ РОЩУ =====
    if u.get("grove_activity") == "grove":
        finish = u.get("grove_activity_finish", 0)
        remaining = int(finish - now)
        
        if remaining <= 0:
            # ВРЕМЯ ВЫШЛО! Генерируем лут и показываем кнопку
    ...
    # ===== ЗАПУСКАЕМ НОВУЮ РОЩУ =====
    duration = GROVE_DURATION  # 90 минут
    u["grove_activity"] = "grove"
    ...
    await update.message.reply_text(
        f"<i>🌳 Лесная роща</i>\n\n"
        f"<i>🍃 Овечка отправилась гулять и вернется через: {time_str}</i>\n"
        f"<i>Напиши /grove чтобы проверить статус!</i>",
        parse_mode="HTML"
    )

async def generate_grove_loot(update: Update, context: ContextTypes.DEFAULT_TYPE, u: dict):
    """Генерирует лут и показывает кнопку для его получения"""
    
    # Проверяем, может лут уже есть
    if u.get("grove_ready", False) and u.get("grove_loot"):
        await show_grove_result(update, context, u)
        return
    ...
    if roll < 70:
        ...
        fruit_name, fruit_field = "🍏 Яблоко", "inv_apple"
    elif roll < 95:
        fruit_name, fruit_field = "🍉 Арбуз", "inv_watermelon"
    else:
        fruit_name, fruit_field = "🥝 Киви", "inv_kiwi"
    ...
    u["grove_ready"] = True
    u["grove_activity"] = None  # Очищаем активность
    u["grove_activity_finish"] = 0
    await save_u(u)
    
    # Показываем кнопку для получения лута
    await show_grove_result(update, context, u)

async def show_grove_result(update: Update, context: ContextTypes.DEFAULT_TYPE, u: dict):
    """Показывает сообщение с кнопкой 'Забрать лут'"""
    loot = u.get("grove_loot")
    if not loot or not u.get("grove_ready", False):
        await update.message.reply_text(
            f"<i>🌳 Лесная роща</i>\n\n"
            f"<i>🍃 Место для отличного поиска различных вещей</i>",
            parse_mode="HTML"
        )
        return
    ...
    await update.message.reply_text(
        f"<i>🌳 Лесная роща</i>\n\n"
        f"<i>🐑 Овечка вернулась и принесла тебе кое что интересное!</i>\n"
        f"<i>Нажми на кнопку, чтобы забрать!</i>",
        reply_markup=kb,
        parse_mode="HTML"
    )

async def grove_collect(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Забирает лут после нажатия кнопки"""
    query = update.callback_query
    u = await get_u(query.from_user.id)
    loot = u.get("grove_loot")
    ...
    fruit_name = loot.get("fruit_name")
    fruit_field = loot.get("fruit_field")
    if fruit_name and fruit_field:
        u[fruit_field] = u.get(fruit_field, 0) + 1
        result_msg = f"🌳 Овечка принесла!\n+{coins} 🐾 и {fruit_name}"
    else:
        result_msg = f"🌳 Овечка принесла!\n+{coins} 🐾"
    
    u["energy"] = max(0, u["energy"] - 20)
    u["grove_ready"] = False
    u["grove_loot"] = None
    await save_u(u)
    ...
    await query.edit_message_text(
        f"<i>🌳 Лесная роща</i>\n\n"
        f"<i>🍃 Место для отличного поиска различных вещей</i>\n"
        f"Напиши /grove чтобы отправить овечку снова!",
        parse_mode="HTML"
    )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Обработчик всех текстовых сообщений для обновления статуса рощи"""
    ...
    if u.get("grove_activity") == "grove":
        ...
        if remaining <= 0:
            ...
            # Если пользователь написал "роща" или "/grove" - показываем таймер
            if "роща" in text or "grove" in text:
                time_str = format_time(remaining)
                await update.message.reply_text(
                    f"<i>🌳 Лесная роща</i>\n\n"
                    f"<i>🍃 Овечка гуляет и обещает вернуться через: {time_str}</i>\n"
                    f"<i>Напиши что-нибудь через {time_str}, чтобы забрать лут!</i>",
                    parse_mode="HTML"
                )
                return
            else:
                # На любое другое сообщение показываем таймер
                time_str = format_time(remaining)
                await update.message.reply_text(
                    f"🌳 Овечка в роще! Вернется через: {time_str}\n"
                    f"Напиши что-нибудь, чтобы проверить статус",
                    parse_mode="HTML"
                )
                return
    ...
    if "роща" in text or "grove" in text:
        ...
        await update.message.reply_text(
            f"<i>🌳 Лесная роща</i>\n\n"
            f"<i>🍃 Овечка отправилась гулять и вернется через: {time_str}</i>\n"
            f"<i>Напиши что-нибудь, чтобы проверить статус!</i>",
            parse_mode="HTML"
        )
        return
    ...
    if "sheep" in text or "овца" in text:
        await sheep_cmd(update, context)
        return
    if "market" in text or "рынок" in text:
        await market_cmd(update, context)
        return
    if "daily" in text or "бонус" in text:
