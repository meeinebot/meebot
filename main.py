...
async def get_u(uid: int) -> dict:
    ...
    if not user:
        ...
        # Миграция: удаляем appetizing_feed если был
        if "talents" in user and user["talents"].get("appetizing_feed", 0) > 0:
            old_level = user["talents"]["appetizing_feed"]
            new_talent = random.choice(["golden_shears", "luxury_wool"])
            user["talents"][new_talent] = user["talents"].get(new_talent, 0) + old_level
            user["talents"]["appetizing_feed"] = 0
        ...
        # Восстановление сытости (1% в час)
...
def get_harvest_time(u: dict, base_time: int = 12 * 3600) -> int:
    shears_level = u.get("talents", {}).get("golden_shears", 0)
    reduction = shears_level * 36 * 60
    return max(6 * 3600, base_time - reduction)

def main_kb(u: dict = None):
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("✂️ Постричь овцу", callback_data="shear"),
         InlineKeyboardButton("✨ Улучшить овцу", callback_data="upgrade")],
...
def profile_text(u: dict) -> str:
    ...
    if now >= u["harvest"]:
        ...
        timer_line = f"⏳ Шерсть будет готова через: {format_time(remaining)}"
    return (
...
async def show_level_up(update: Update, context: ContextTypes.DEFAULT_TYPE, u: dict, level: int):
    if not u.get("pending_apple_given", False):
        u["inv_apple"] = u.get("inv_apple", 0) + 1
        u["pending_apple_given"] = True
        await save_u(u)
        apple_msg = " +1 🍏 Яблоко"
    else:
        apple_msg = ""
    ...
    if level >= 20:
        await update.message.reply_text(
            f"🎉 Овечка достигла {level} уровня!{apple_msg}",
...
async def learn_talent(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if level >= 20:
        await query.answer("🌟 Овечка уже достигла максимального уровня!", show_alert=True)
        u["pending_level_up"] = False
        await save_u(u)
    ...
    await query.answer(f"⚡️ Изучен новый талант! {talent_names[chosen]}. ({old_level} → {new_level})", show_alert=True)
    await query.edit_message_text(profile_text(u), reply_markup=main_kb(u))
...
async def upgrade(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    u["pending_apple_given"] = False
    await save_u(u)
    
    await query.answer("🎉 Уровень успешно повышен!", show_alert=True)
    
    new_level = u.get("level", 0)
    next_next_level = new_level + 1
    new_cost = 10 + (new_level - 1) * 3
    
    text = f"💪 Улучшение овечки\n\n⭐️ Уровень: {new_level} → {next_next_level}\n💰 Стоимость: {new_cost} 🧶"
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("✨ Улучшить овцу", callback_data="upgrade")],
        [InlineKeyboardButton("⬅️ Назад", callback_data="back"),
         InlineKeyboardButton("ℹ️ Гид", callback_data="upgrade_guide")]
    ])
    await query.edit_message_text(text, reply_markup=kb)

async def upgrade_guide(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer("ℹ️ Гид пока в разработке!", show_alert=True)
...
async def use_fruit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer(f"{emoji} Овечка съела фрукт! 🌿 +{restore} к сытости.", show_alert=True)
...
async def sell_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if u["wool"] <= 0:
        ...
        return
    luxury_level = u.get("talents", {}).get("luxury_wool", 0)
    base_price = 10 + luxury_level
    v = u["wool"] * base_price
    u["balance"] += v
    u["wool"] = 0
    await save_u(u)
...
async def shear(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    u["wool"] += gain
    harvest_time = get_harvest_time(u)
    u["harvest"] = now + harvest_time
...
def main():
    ...
    application.add_handler(CallbackQueryHandler(upgrade_guide, pattern="^upgrade_guide$"))
