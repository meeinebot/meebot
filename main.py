...
async def shear(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Проверяем, не стрижётся ли уже
    if u.get("shearing", 0) == 1:
        # Если стрижка уже идёт, показываем таймер
    ...
    if u["shearing"]:
        if now >= u["s_finish"]:
            # СТРИЖКА ЗАВЕРШЕНА - ВЫДАЁМ ШЕРСТЬ
...
async def grove_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ===== ПРОВЕРЯЕМ, НЕ СТРИЖЁТСЯ ЛИ ОВЕЧКА (ИЛИ ЕСТЬ НЕЗАБРАННАЯ ШЕРСТЬ) =====
    if u.get("shearing", 0) == 1 or u.get("wool", 0) > 0:
        await update.message.reply_text("✂️ Овечка занята. ⏳ Дождись её прихода!")
        return
...
async def grove_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ===== ПРОВЕРЯЕМ, НЕ СТРИЖЁТСЯ ЛИ ОВЕЧКА (ИЛИ ЕСТЬ НЕЗАБРАННАЯ ШЕРСТЬ) =====
    if u.get("shearing", 0) == 1 or u.get("wool", 0) > 0:
        await query.answer("✂️ Овечка занята. ⏳ Дождись её прихода!", show_alert=True)
        return
