...
async def show_wolf_inline(update: Update, context: ContextTypes.DEFAULT_TYPE, u: dict):
    """Показывает волка, редактируя текущее сообщение"""
...
async def check_and_activate_wolf(u: dict) -> bool:
    """Проверяет, нужно ли активировать волка"""
    now = time.time()
    last_offer = u.get("wolf_last_offer", 0)
    
    if now - last_offer >= 24 * 3600 and not u.get("wolf_active", False):
...
async def sheep_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await check_and_activate_wolf(u)
    
    if u.get("wolf_active", False):
        # Создаём новое сообщение для волка, так как sheep_cmd вызывается без query
        item = u.get("wolf_item")
        if item:
            text = f"🐺 Странный торговец.\n«Товары на любой вкус»\n{item['name']} – {item['price']} 🐾"
            if u['balance'] < item['price']:
...
async def clan_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Если у игрока есть клан — показываем его
    if u.get("clan_id"):
        clan = clans.find_one({"_id": u["clan_id"]})
        if clan:
            members_count = len(clan.get("members", []))
            text = f"⭐️ {clan['name']}.\n👥 Участники: {members_count}/10"
            kb = InlineKeyboardMarkup([
                [InlineKeyboardButton("📢", callback_data="clan_announce"),
                 InlineKeyboardButton("👤", callback_data="clan_members"),
                 InlineKeyboardButton("🎯", callback_data="clan_goals"),
                 InlineKeyboardButton("📦", callback_data="clan_chest")],
                [InlineKeyboardButton("⚙️ Настройки", callback_data="clan_settings")]
            ])
            await update.message.reply_text(text, reply_markup=kb)
            return
    
    # Если нет клана — показываем меню создания
    kb = InlineKeyboardMarkup([
...
async def clan_create_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    u["clan_creating"] = True
    u["clan_temp_name"] = None
    await save_u(u)
    
    text = "🤝 Создание клана.\nОтветь на сообщение с названием. (От 3 до 10 символов)"
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("⬅️ Назад", callback_data="clan_back")]
    ])
    await query.edit_message_text(text, reply_markup=kb)

async def clan_handle_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Обработка введённого названия клана"""
    user_id = update.effective_user.id
    u = await get_u(user_id)
    name = update.message.text.strip()
    
    # Проверяем, что игрок в режиме создания
    if not u.get("clan_creating", False):
        await update.message.reply_text("❌ Используй /clan для создания клана.")
        return
    
    # Проверяем длину
    if len(name) < 3 or len(name) > 10:
        await update.message.reply_text("❌ Название должно быть от 3 до 10 символов. Попробуй снова.")
        return
    
    # Проверяем, не занято ли название
    if clans.find_one({"name": name}):
        await update.message.reply_text("❌ Такое название уже занято! Попробуй другое.")
        return
    
    u["clan_temp_name"] = name
    await save_u(u)
    
    # Показываем подтверждение
    text = f"⭐️ {name}.\n💰 Стоимость: 499 🐾"
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("🤝 Создать", callback_data="clan_confirm")],
        [InlineKeyboardButton("⬅️ Назад", callback_data="clan_create")]
    ])
    await update.message.reply_text(text, reply_markup=kb)

async def clan_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Подтверждение создания клана"""
    query = update.callback_query
    u = await get_u(query.from_user.id)
    
    name = u.get("clan_temp_name")
    if not name:
        await query.answer("❌ Ошибка! Название не найдено.", show_alert=True)
        return
    ...
    # Создаём клан
    clan_id = f"clan_{int(time.time())}_{u['id']}"
    clan_data = {
        "_id": clan_id,
        "name": name,
        "leader_id": u['id'],
    ...
    await query.edit_message_text(text, reply_markup=kb)
    await query.answer("✅ Клан успешно создан!", show_alert=True)

async def clan_back(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Назад в главное меню кланов"""
    query = update.callback_query
    u = await get_u(query.from_user.id)
    ...
    await query.edit_message_text("⭐️ Ты ещё не в клане!\nВыбери, что хочешь сделать.", reply_markup=kb)

async def clan_create_back(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Назад к вводу названия"""
    query = update.callback_query
    u = await get_u(query.from_user.id)
...
async def clan_placeholder(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Заглушка для всех кнопок клана"""
...
async def shear(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if u['shearing']:
        if now >= u['s_finish']:
            if u.get("buff_mango_wool", 0) or u.get("buff_coconut_wool", 0):
...
def main():
    ...
    # Обработка сообщений (для названия клана)
    application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, clan_handle_name))
    ...
    # Кланы
    application.add_handler(CallbackQueryHandler(clan_create_start, pattern="^clan_create$"))
    application.add_handler(CallbackQueryHandler(clan_confirm, pattern="^clan_confirm$"))
    application.add_handler(CallbackQueryHandler(clan_back, pattern="^clan_back$"))
    application.add_handler(CallbackQueryHandler(clan_create_back, pattern="^clan_create_back$"))
    application.add_handler(CallbackQueryHandler(clan_placeholder, pattern="^clan_"))
    application.add_handler(CallbackQueryHandler(clan_placeholder, pattern="^clan_find$"))
