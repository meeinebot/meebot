...
async def wolf_refuse(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Проверяем иммунитет (1 раз)
    if u.get("immunity", False):
        # Иммунитет сработал - ничего не крадём
        u["immunity"] = False
    ...
    # Если не украл овечку - крадёт сытость
...
async def owl_refuse(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if random.random() < 0.5:
        ...
        # Сова не украла копытца - крадёт сытость
        satiety_loss = random.randint(10, 30)  # Диапазон для совы
        u["satiety"] = max(0, u.get("satiety", 100) - satiety_loss)
        await save_u(u)
        await query.answer("🦉 Мудрая сова улетела в глубь леса!", show_alert=True)
...
async def shear(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    satiety = u.get("satiety", 100)
    if satiety < 5:
        await query.answer("❌ Недостаточно сытости!", show_alert=True)
        return
    ...
    if u["shearing"]:
        if now >= u["s_finish"]:
            ...
            # Показываем на 1 минуту меньше (4:59)
            actual_rem = rem - 60
            if actual_rem < 0:
                actual_rem = 0
...
async def grove_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    satiety = u.get("satiety", 100)
    if satiety < 10:
        await update.message.reply_text("❌ Недостаточно сытости! Нужно 10 🌿")
        return
...
async def grove_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    satiety = u.get("satiety", 100)
    if satiety < 10:
        await query.answer("❌ Недостаточно сытости! Нужно 10 🌿", show_alert=True)
        return
