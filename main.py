...
GROVE_DURATION = 89 * 60  # 1 час 29 минут (89 минут)
...
async def reave_time_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Сбрасывает рощу до 15 секунд (для тестов)"""
    if update.effective_user.id != ADMIN_ID:
        await update.message.reply_text("❌ У вас нет прав!")
        return
    args = context.args
...
async def daily_claim(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if reward["type"] == "coins":
        ...
        await save_u(u)
        await query.answer(f"💰 +{amount} 🐾", show_alert=True)
...
async def daily_fruit_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    u["daily_claimed_today"] = True
    await save_u(u)
    await query.answer(f"💰 +{fruit_name}", show_alert=True)
    await query.edit_message_text(
        f"<i>🎯 Ежедневный бонус</i>\n<i>⭐️ Серия наград: {u.get('daily_streak', 0)}</i>",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("💰 Получить награду", callback_data="daily_claim")]
...
async def wolf_refuse(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if random.random() * 100 < steal_chance:
        ...
        await save_u(u)
        await query.answer(f"🐺 Схватил {old_skin} и сбежал!", show_alert=True)
    else:
        await query.answer("🐺 Ушёл в лес!", show_alert=True)
    u["wolf_active"] = False
    u["wolf_item"] = None
    u["next_visitor"] = "owl"
    await save_u(u)
...
async def owl_refuse(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if random.random() < 0.5:
        ...
        await save_u(u)
        await query.answer(f"🦉 Сова украла {stolen} 🐾", show_alert=True)
    else:
        await query.answer("🦉 Сова улетела!", show_alert=True)
    u["owl_active"] = False
    u["owl_advice"] = None
...
async def shear(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if u.get("grove_activity") == "grove":
        ...
        if now < finish:
            await query.answer("🌲 Овечка занята!", show_alert=True)
            return
    ...
    if u["shearing"]:
        if now >= u["s_finish"]:
            ...
            if u.get("buff_coconut_wool", 0):
                u["buff_coconut_wool"] = 0
            await save_u(u)
            await query.answer(f"🐑 +🧶 {gain}", show_alert=True)
            await query.edit_message_text(profile_text(u), reply_markup=main_kb(u), parse_mode="HTML")
        else:
            rem = int(u["s_finish"] - now)
            time_str = format_time(rem)
            await query.answer(f"✂️ Осталось: {time_str}", show_alert=True)
    elif now < u["harvest"]:
        rem = int(u["harvest"] - now)
        time_str = format_time(rem)
        await query.answer(f"❌ Подожди: {time_str}", show_alert=True)
    else:
        u["shearing"] = 1
        u["s_finish"] = now + 300
        await save_u(u)
        await query.answer("✂️ Стрижка 5 минут", show_alert=True)
...
async def generate_grove_loot(update: Update, context: ContextTypes.DEFAULT_TYPE, u: dict):
    """Генерирует лут и СОХРАНЯЕТ В БД"""
