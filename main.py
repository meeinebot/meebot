...
async def get_u(uid: int) -> dict:
    ...
    if not user:
        ...
        # Восстановление энергии (1% каждые 5 минут)
...
async def wolf_refuse(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Отнимаем энергию при отказе (100% шанс, если не купили)
...
async def shear(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Проверяем энергию (достаточно ли для начала)
    energy = u.get("energy", 100)
    if energy < 5:
...
async def grove_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Проверяем энергию (достаточно ли для отправки)
    energy = u.get("energy", 100)
    if energy < 20:
    ...
    if u.get("shearing", 0) == 1:
        if now < u.get("s_finish", 0):
            rem = int(u["s_finish"] - now)
            time_str = format_time(rem)
            await update.message.reply_text(
                f"<i>🌳 Лесная роща</i>\n\n"
                f"<i>✂️ Овечка сейчас стрижётся!</i>\n"
                f"<i>⏳ Подожди: {time_str}</i>",
                parse_mode="HTML"
            )
            return
        else:
    ...
    # Отправляем овечку (НЕ снимаем энергию здесь)
...
async def grove_collect(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Снимаем энергию ПОСЛЕ возврата из рощи
