...
def check_fire(user_id):
    ...
    # Если игрок не в раунде - пропускаем проверку
    if player[4] == 0:
        return "not_in_round"
    
    # Если время не установлено - ставим текущее (защита от бага)
...
async def profile_handler(message: types.Message):
    ...
    # Проверяем огонь только если в раунде
    if in_round == 1:
...
async def create_round(call: types.CallbackQuery):
    ...
    if call.message.chat.type in ["group", "supergroup"]:
        ...
        # ЛС - только одиночный режим
        player = get_player(user_id)
        if player[4] == 1:  # in_round
