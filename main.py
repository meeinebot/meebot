...
GROVE_DURATION = 90 * 60

GROVE_LOOT_TABLE = [
    (25, "🍏 Яблоко", "inv_apple"),
    (25, "🫐 Черника", "inv_blueberry"),
    (15, "🥭 Манго", "inv_mango"),
    (15, "🍉 Арбуз", "inv_watermelon"),
    (5, "🥝 Киви", "inv_kiwi"),
    (5, "🥥 Кокос", "inv_coconut"),
]
...
async def get_u(uid: int) -> dict:
    ...
    if not user:
        ...
        # Обновляем структуру, если есть новые поля
...
async def check_and_show_visitor(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    """Проверяет, есть ли активный волк или сова, и показывает их"""
    user_id = update.effective_user.id
    u = await get_u(user_id)
    
    # Проверяем, не активировался ли визитёр
...
async def check_visitor_on_action(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    """Проверяет визитора при нажатии инлайн-кнопки"""
    query = update.callback_query
    u = await get_u(query.from_user.id)
    await check_and_activate_visitor(u, update, context)
    if u.get("wolf_active", False):
...
async def generate_grove_loot(update: Update, context: ContextTypes.DEFAULT_TYPE, u: dict):
    ...
    cumulative = 0
    for chance, name, field in GROVE_LOOT_TABLE:
        cumulative += chance
        if roll < cumulative:
            fruit_name = name
            fruit_field = field
    ...
    loot = {"coins": coins, "fruit_name": fruit_name, "fruit_field": fruit_field}
    u["grove_loot"] = loot
    u["grove_ready"] = True
    await save_u(u)
    await show_grove_result(update, context, u)
...
async def grove_collect(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if fruit_name and fruit_field:
        ...
        result_msg = f"🌳 Вот что тебе принесла твоя овечка! +{coins} 🐾 и {fruit_name}"
    else:
        result_msg = f"🌳 Вот что тебе принесла твоя овечка! +{coins} 🐾"
